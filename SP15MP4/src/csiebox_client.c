...
static int login(csiebox_client* client) {
	...
	while(1) {
		...
		recv_message(client->conn_fd, &busy, sizeof(busy));
...
static void sync_file_data(csiebox_client* client, char* path) {
	...
	if ((stat.st_mode & S_IFMT) == S_IFDIR) {
		...
		if (fd < 0) {
			...
			// lock the file in client
			while (flock(fd, LOCK_SH | LOCK_NB) != 0) {
				fprintf(stderr, "client block\n");
				sleep(3);
			};
			// first send message to make server classify the file
			send_message(client->conn_fd, &file, sizeof(file));
			// send message check if is blocked
			int request = 1;
			csiebox_protocol_header block;
			while (1) {
				send_message(client->conn_fd, &request, sizeof(int));
				recv_message(client->conn_fd, &block, sizeof(block));
				if (block.res.status == CSIEBOX_PROTOCOL_STATUS_BLOCKED) {
...
static void rm_file(csiebox_client* client, char* path, int is_dir) {
	char* relative = convert_to_relative_path(client, path);
	if (!relative) {
		fprintf(stderr, "convert relative fail\n");
		return;
	}
	if (is_dir) {
