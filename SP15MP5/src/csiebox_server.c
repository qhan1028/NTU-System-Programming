...
void csiebox_server_init(csiebox_server** server, int argc, char** argv) {
	...
	int fd = server_start();
	if (fd < 0) {
		fprintf(stderr, "server_start fail\n");
		free(tmp);
		return;
	}
...
int csiebox_server_run(csiebox_server* server) { 
	...
	// daemonize
	if (server->arg.daemonize) daemonize(server);
	// socket init
	...
	for (int i = 0 ; i < tmax ; i++) {
		...
		args[i].client.fd = -1; args[i].client.logout = 0;
		args[i].status.idle = 1; args[i].status.in_thread = in_thread; args[i].status.fdset = &master;
		pthread_create(&tid[i], NULL, thread_function, &args[i]);
	}
	fprintf(stderr, "--- fdset, thread pool init success\n");
	// busy threads count init
	struct timeval tv;
	tv.tv_sec = 1; tv.tv_usec = 0;
	//pthread_mutex_init(&busy_threads->lock, NULL);
	fprintf(stderr, "--- busy thread count init success\n");
	// create fifo
	sprintf(fifo_path, "%s/fifo.%d", server->arg.run_path, getpid());
	...
	if (signal(SIGUSR1, sig_func) == SIG_ERR ||
		signal(SIGTERM, sig_func) == SIG_ERR || 
		signal(SIGINT, sig_func) == SIG_ERR) {
		fprintf(stderr, "signal construct error\n");
		return 0;
	} else fprintf(stderr, "--- signal init success\n");
	// start run
			...
			// handle request from connected socket fd
				...
				for (int i = 0 ; i < tmax ; i++) {
					...
					if (args[i].status.idle && in_thread[fd] == 0) {
						...
						in_thread[fd] = 1;
						args[i].client.fd = fd; args[i].status.idle = 0;
						busy = 0;
						break;
					}
				}
...
static void daemonize(csiebox_server *server) {
	...
	if (pid < 0) { // error
		...
		exit(1);
	} else if (pid > 0) { // parent, create csiebox_server.pid
		char pid_path[PATH_MAX];
		sprintf(pid_path, "%s/csiebox_server.pid", server->arg.run_path);
	...
	if (chdir("/tmp") < 0) fprintf(stderr, "can't switch to tmp dir\n");
	fprintf(stderr, "\n========== daemonize success\n");
}
...
static void sig_func(int signo) {
	if (signo == SIGUSR1) {
		...
		uint32_t result = htonl(busy_threads_count);
		write(fifo, &result, sizeof(uint32_t));
		close(fifo);
